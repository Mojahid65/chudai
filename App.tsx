...
import { Twitter, Linkedin, Github, Coffee, Heart } from "lucide-react";
...
const App: React.FC = () => {
  const [currentView, setCurrentView] = useState<'home' | 'creators' | 'donate'>('home');
    ...
    <GridBackground>
          ...
          <nav className="flex items-center gap-2 md:gap-6">
            <button 
              ...
              className={`text-sm font-medium transition-all duration-200 ${
                currentView === 'home' 
                  ? 'text-violet-600 bg-violet-50 px-3 py-1.5 rounded-full' 
            ...
            <button 
              ...
              className={`text-sm font-medium transition-all duration-200 flex items-center gap-1 ${
                currentView === 'donate' 
                  ? 'text-amber-600 bg-amber-50 px-3 py-1.5 rounded-full' 
                  : 'text-slate-500 hover:text-amber-600'
              }`}
            >
              <Coffee className={`w-3.5 h-3.5 ${currentView === 'donate' ? 'fill-amber-600' : ''}`} />
              Donate
            </button>
          </nav>
        </div>
      ...
      <main className="flex-grow w-full max-w-4xl mx-auto px-4 py-8 md:py-12 flex flex-col items-center">
        ...
        {currentView === 'home' && (
          <>
            {/* HERO */}
            <div className="text-center mb-10 space-y-4 animate-in fade-in duration-500">
            ...
            {!selectedFile && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full mt-12 animate-in fade-in slide-in-from-bottom-4 delay-200 duration-700 fill-mode-both">
                {[
                  { 
            ...
            {!selectedFile && history.length === 0 && (
               <div className="w-full mt-20 mb-20 animate-in fade-in slide-in-from-bottom-8 delay-300 duration-700 fill-mode-both">
        ...
        {currentView === 'creators' && <CreatorsPage />}
