...
function useStepItem() {
  ...
  if (ctx === undefined) {
     // Return a default or throw depending on usage.
     // For robustness if used outside context unknowingly (though it shouldn't be):
     return { step: 0, state: 'inactive' as StepState, isDisabled: false, isLoading: false };
  }
  return ctx;
}
  ...
  return (
    <StepItemContext.Provider value={{ step, state, isDisabled: disabled, isLoading }}>
      <div
        ...
        className={cn(
          'group/step flex items-center justify-center group-data-[orientation=horizontal]/stepper-nav:flex-row group-data-[orientation=vertical]/stepper-nav:flex-col data-[orientation=horizontal]:[&:not(:last-child)]:flex-1',
          className,
        )}
        data-state={state}
